...
serve(async (req) => {
  ...
  try {
    ...
    // Balanced prompt: focus on stable identity features, ignore transient variations (facial hair, lighting, angle, glasses)
    const systemPrompt = `You are a face-verification assistant. Compare two face images and decide if they belong to the SAME PERSON. Focus on STABLE identity features: face/skull shape, eye shape and spacing, nose shape, mouth/lip shape, ear shape, jawline, permanent scars/moles. IGNORE transient variations: facial hair (beard/moustache/stubble), hairstyle, glasses, lighting, angle, expression, skin-tone shifts from lighting, image quality. Two images of the same person with different facial hair or lighting MUST still be considered a match. Only return match=false when core bone structure or eye/nose/mouth geometry clearly differ. Return ONLY a single JSON object: {"match": boolean, "confidence": 0-100, "reason": "short"}. No markdown, no extra text.`;

    const userPrompt = `Compare these two images (first=stored reference, second=live capture). Decide if the SAME PERSON is in both, IGNORING facial hair, glasses, lighting, angle, and expression. Return ONLY JSON: {"match": true|false, "confidence": 0-100, "reason": "brief"}. If images are unreadable, return match=false with reason "invalid-image".`;
